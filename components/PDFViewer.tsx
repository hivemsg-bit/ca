...
export const PDFViewer: React.FC<PDFViewerProps> = ({ url, title, onClose }) => {
  ...
  const handleDirectDownload = async () => {
    ...
    try {
      ...
      // We try target="_self" first to respect "no new tab", 
      // but if the browser enforces a view (not download), it might navigate. 
      // Safest fallback for "Download" button is usually to just let the browser handle it.
      
      const link = document.createElement('a');
      link.href = url;
      link.target = '_blank'; // Fail-safe: ensures we don't crash the app if it navigates
