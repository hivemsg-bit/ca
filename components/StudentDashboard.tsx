...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user: any) => {
      ...
      try {
        ...
        // 2. Real-time Tests (Live Updates from Admin)
        ...
        unsubTests = onSnapshot(qTests, (snapshot) => {
            ...
            // Filter based on student course
            const relevantTests = allTests.filter((t: any) => t.level === (profileData?.course || 'CA Final'));
            setAvailableTests(relevantTests);
        });

        // 3. Real-time Materials (Live Updates from Admin)
        ...
        // 4. Real-time Submissions (Live Updates when Checked)
  ...
  const uploadFile = async (file: File): Promise<string> => {
    try {
      ...
      // Critical: Set content disposition for files uploaded by students too
      const metadata = {
  ...
  const handleDirectDownload = async (url: string, filename: string, id: string) => {
    setDownloadingId(id);
    try {
      // Try fetch for standard download
      ...
      // Fallback: Create a direct link download
      // We use target="_blank" as a fail-safe to ensure the file opens even if CORS blocks headers
      const link = document.createElement('a');
      link.href = url;
      link.target = "_blank";
      link.download = filename;
