...
import { 
  ...
  Eye, CheckCircle2, ClipboardCheck, Calendar
} from 'lucide-react';
...
type TabType = 'dashboard' | 'students' | 'tests' | 'library' | 'evaluations' | 'payments' | 'settings';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ plans, onUpdatePlans, onLogout, onBack }) => {
  ...
  const [newStudent, setNewStudent] = useState({ 
    ...
    plan: 'Detailed Test Series', planStatus: 'Active' 
  });
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user: any) => {
      if (user) {
        ...
        const errorHandler = (error: any) => {
            ...
            // alert("Database Connection Error. Please check your internet or Firebase Rules.");
            setLoading(false);
        };
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    const studentData = { ...newStudent, expiryDate, joinDate, progress: 0 };

    try {
      await addDoc(collection(db, "students"), studentData);
      alert('Student added to Database!');
      setShowAddStudentModal(false);
      setNewStudent({ name: '', email: '', phone: '', course: 'CA Final', plan: 'Detailed Test Series', planStatus: 'Active' });
    } catch (e: any) {
  ...
  const handleFileUpload = async (file: File): Promise<string> => {
    try {
      ...
      // CRITICAL: Set content disposition to attachment to force download
      const metadata = {
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      <aside className="hidden lg:flex w-64 bg-brand-dark p-6 flex-col shrink-0 min-h-screen sticky top-0 overflow-y-auto z-20">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <SidebarItem id="students" icon={Users} label="Student Management" />
          <SidebarItem id="tests" icon={FileText} label="Test Repository" />
          <SidebarItem id="evaluations" icon={ClipboardCheck} label="Evaluations" />
          <SidebarItem id="library" icon={Library} label="Material Library" />
          <SidebarItem id="payments" icon={CreditCard} label="Transactions" />
        ...
        {activeTab === 'dashboard' && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-up">
              {[
                ...
                { label: 'Live Tests', value: tests.filter(t => t.status === 'Live').length, icon: FileText, color: 'text-green-500', bg: 'bg-green-50' },
                { label: 'Pending Evaluations', value: submissions.filter(s => s.status === 'Pending').length, icon: ClipboardCheck, color: 'text-orange-500', bg: 'bg-orange-50' },
                { label: 'Library items', value: materials.length, icon: Library, color: 'text-purple-500', bg: 'bg-purple-50' },
              ].map((stat, i) => (
             ...
             <div className="animate-fade-up">
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                   <div className="p-6 border-b border-slate-100">
                      <h3 className="font-bold text-brand-dark">Student Submissions</h3>
                      <p className="text-xs text-brand-dark/40">Download answers, evaluate, and upload checked copies.</p>
                   </div>
                      ...
                      <table className="w-full text-left">
                         <thead className="bg-slate-50">
                            <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                               ...
                               <th className="px-6 py-4 text-right">Evaluate</th>
                            </tr>
                         </thead>
                         <tbody className="divide-y divide-slate-50">
                            {submissions.map((sub) => (
                               <tr key={sub.id} className="hover:bg-slate-50/50">
                                  ...
                                  <td className="px-6 py-4">
                                     <span className={`text-[10px] font-black uppercase px-2 py-1 rounded ${sub.status === 'Evaluated' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                                        {sub.status}
                                     </span>
                                  </td>
                                  <td className="px-6 py-4">
                                     ...
                                     {sub.status === 'Evaluated' ? (
                                        ...
                                        <div className="relative inline-block">
                                            ...
                                            >
                                                ...
                                                {evaluatingSubmissionId === sub.id ? 'Uploading...' : 'Upload Checked Copy'}
                                            </button>
                                        </div>
                                     )}
                                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-fade-up">
            ...
            <form onSubmit={handleAddStudent} className="space-y-4">
              ...
              <Button type="submit" fullWidth>Add Student</Button>
            </form>
          </div>
        </div>
      )}
